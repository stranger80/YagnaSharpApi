...
namespace YagnaSharpApi.Engine
{
    ...
    public class Executor : IDisposable
    {
        ...
        [Obsolete]
        public async IAsyncEnumerable<GolemTask<TData, TResult>> SubmitAsync<TData, TResult>(Func<WorkContext, IAsyncEnumerable<GolemTask<TData, TResult>>, IAsyncEnumerable<Command>> worker, IEnumerable<GolemTask<TData, TResult>> data)
        {
            this.OnExecutorEvent?.Invoke(this, new SubmitStarted());
        ...
        /// <returns></returns>
        [Obsolete]
        protected async Task WorkerStarter<TData, TResult>(Func<WorkContext, IAsyncEnumerable<GolemTask<TData, TResult>>, IAsyncEnumerable<Command>> worker, IEnumerable<GolemTask<TData, TResult>> data, SmartQueue<TData, TResult> smartQueue, CancellationToken cancellationToken)
        {
            System.Diagnostics.Debug.WriteLine("Starting WorkerStarter...");
            do
            {
        ...
        protected async Task DoWork<TData, TResult>(BufferedAgreement bufferedAgreement, Func<WorkContext, IAsyncEnumerable<GolemTask<TData, TResult>>, IAsyncEnumerable<Script>> worker, IEnumerable<GolemTask<TData, TResult>> data, SmartQueue<TData, TResult> smartQueue)
        {
            ...
            if (!await this.ProcessBatchesAsync<TData, TResult>(commandGenerator, bufferedAgreement, activity, lastTask))
                return;
        ...
        protected async Task<bool> ProcessBatchesAsync<TData, TResult>(IAsyncEnumerable<Script> commandGenerator, 
            BufferedAgreement bufferedAgreement, ActivityEntity activity, Box<GolemTask<TData, TResult>> lastTaskBox = null)
        {
        ...
        protected async Task PerformImplicitInitAsync(WorkContext ctx, BufferedAgreement bufferedAgreement, ActivityEntity activity)
        {
            async IAsyncEnumerable<Script> ImplicitInitAsync()
            {
                ...
                script.Add(new DeployStep());
                script.Add(new StartStep());
                yield return script;
            }
            await this.ProcessBatchesAsync<object, string>(ImplicitInitAsync(), bufferedAgreement, activity);

        }


        [Obsolete]
        protected async Task DoWork<TData, TResult>(BufferedAgreement bufferedAgreement, Func<WorkContext, IAsyncEnumerable<GolemTask<TData, TResult>>, IAsyncEnumerable<Command>> worker, IEnumerable<GolemTask<TData, TResult>> data, SmartQueue<TData, TResult> smartQueue)
        {
            this.OnExecutorEvent?.Invoke(this, new WorkerStarted(bufferedAgreement.Agreement.AgreementId));
