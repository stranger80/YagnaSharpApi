...
{
    public class DebugMessageHandler : PipeMessageHandler
    {
        /// <summary>
        ...
        public DebugMessageHandler(IDuplexPipe pipe, IJsonRpcMessageTextFormatter formatter)
            : base(pipe, formatter)
        {
        ...
        /// <param name="formatter">The formatter used to serialize messages. Only UTF-8 formatters are supported.</param>
        public DebugMessageHandler(PipeWriter? writer, PipeReader? reader, IJsonRpcMessageTextFormatter formatter)
        ...
        /// <param name="formatter">The formatter used to serialize messages. Only UTF-8 formatters are supported.</param>
        public DebugMessageHandler(Stream writer, Stream reader, IJsonRpcMessageTextFormatter formatter)
            : base(writer, reader, formatter)
        {
            this.CommonConstructor();
        }
        ...
        protected override void Write(JsonRpcMessage content, CancellationToken cancellationToken)
        {
            ...
            var req = Encoding.UTF8.GetString(buffer.WrittenSpan.ToArray());
